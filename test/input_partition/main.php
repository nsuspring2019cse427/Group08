...
class Main extends CI_Controller {
	...
	public function setParam(){
		$email = "sdfdsfggfafsafafdsfsafdsafdsafsdafsdafdsa@b.com";
		$pass = 11254233;
		$fName = "Rupom";
		$lName = "Hasan";
		$addr = 'Jagannathpur, Dhaka-1229';
		$pHNumber = "141417585678678675";

		$this->ShowTest($email, $pass);
		$this->showUser($fName, $lName, $addr, $pHNumber);
		
	}

	public function ShowTest($email, $pass){
		...
		if(strlen($pass)>=12){
			...
			echo $pass."==> Perfect Password";
		}
			
	}
 
 	public function showUser($f, $l, $a, $p){
 		echo("</br>");
		if(strlen($f)>=12){
			echo $f."  ==> invalid First Name";
		}elseif(strlen($f)<1){
			echo $f."  ==> invalid First Name";
		}else{
			echo $f." ==> Perfect First name";
		}	

		echo("</br>");
		if(strlen($l)>=12){
			echo $l."  ==> invalid Last Name";
		}elseif(strlen($l)<1){
			echo $l."  ==> invalid Last Name";
		}else{
			echo $l." ==> Perfect Last name";
		}	

		echo("</br>");
		if(strlen($a)>=12){
			echo $a."  ==> invalid Address";
		}elseif(strlen($a)<6){
			echo $a."  ==> invalid Address";
		}else{
			echo $a." ==> Perfect Address";
		}	

		echo("</br>");
		if(strlen($p)>=12){
			echo $p."  ==> invalid Phone Number";
		}elseif(strlen($p)<6){
			echo $p."  ==> invalid Phone Number";
		}else{
			echo $p." ==> Perfect Phone Number";
		}	
 	}
	...
	public function validate_credentials(){
		...
		if($this->model_users->can_log_in()){
			...
			$this->form_validation->set_message('validate_credentials','<b style="background: #FFAAAA;padding: 5px 15px;">Incorrect UserName/PassWord.</b>');
			return false;
		}
	}
	...
	public function food_provide_validation(){
		$this->load->library('form_validation');
		$this->form_validation->set_rules('company_name','Company_Name','required|min_length[10]|max_length[25]');
	...
	public function show_foodprovider_id() {
